...
__device__ void Run_Neural_Net(CraftState* C, bool Do_Activation, int ID_Neurons, int ID_Weights)
{
	...
	for (unsigned int Input = 0; Input < LAYER_SIZE_INPUT; Input++)
	{
		for (unsigned int Output = LAYER_SIZE_INPUT; Output < LAYER_SIZE_INPUT + NEURONS_PER_LAYER; Output++)
		{
			...
			C->Neuron[2 * NEURON_COUNT * Output + ID_Neurons] += C->Neuron[2 * CRAFT_COUNT * Input + ID_Neurons] * C->Weight[CRAFT_COUNT * Weight_Index + ID_Weights];
		}
	}
	...
	for (unsigned int i = 0; i < NEURONS_PER_LAYER; i++)
	{
		...
		if (Do_Activation)
			RELU_Activate(C->Neuron[Index * 2 * CRAFT_COUNT + ID_Neurons]);
	}
	...
	for (unsigned int Layer = 1; Layer < LAYER_AMOUNT_HIDDEN; Layer++)
	{
		for (unsigned int Input = 0; Input < NEURONS_PER_LAYER; Input++)
		{
			...
			{
				unsigned int Output_Index = Output + LAYER_SIZE_INPUT + Layer * NEURONS_PER_LAYER;
				unsigned int Input_Index = Input + LAYER_SIZE_INPUT + (Layer - 1) * NEURONS_PER_LAYER;
		...
		for (unsigned int Output = 0; Output < NEURONS_PER_LAYER; Output++)
		{
			unsigned int Index = Output + LAYER_SIZE_INPUT + Layer * NEURONS_PER_LAYER;
	...
	for (unsigned int Input = 0; Input < NEURONS_PER_LAYER; Input++)
	{
		...
		{
			unsigned int Output_Index = Output + LAYER_SIZE_INPUT + NEURONS_PER_LAYER * NEURONS_PER_LAYER;
			unsigned int Input_Index = Input + LAYER_SIZE_INPUT + (LAYER_AMOUNT_HIDDEN - 1) * NEURONS_PER_LAYER;
...
__device__ void NeuralNet(CraftState *C, GraphicsObjectPointer Buffer, int ID_Opponent, int ID_Craft, int ID_Weight)
{
